...
export default class CompletedRequestService extends FirebaseCollection<CompletedRequest> {
	...
	private async getCompletedRequestOf(
		id: string
	): Promise<OperationResult<Result<CompletedRequest>[]>> {
		try {
	...
	public async getCurrentUserCompletedRequests(): Promise<
		OperationResult<Result<CompletedRequest>[]>
	> {
